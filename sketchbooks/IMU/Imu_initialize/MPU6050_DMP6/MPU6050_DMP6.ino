...
accel_t_gyro.value.x_gyro= accel_t_gyro.value.x_gyro-778;
...
// ROHE DATEN VERARBEITEN
  
  //Winkelberechnung 

angle = angle + ((accel_t_gyro.value.x_gyro/131.0)*0.1);           // laut Datenblatt muss man durch 131 teilen..mal 0.1 entspricht einer ungenauen Integration (Rechteck unter Kurve)
angle = angle + ((accel_t_gyro.value.y_gyro/131.0)*0.1);              // Anpassung der Koordinaten muss natürlich auf allen Achsen erfolgen
angle = angle + ((accel_t_gyro.value.z_gyro/131.0)*0.1);
...
m++;
    ...
    //Ausgleichen des natürlichen Drifts des Gyros
...
a=(angle-angle_2)*t*t;
c=a*dt;
b=c+((angle-angle_2)*2*t);

Serial.println("Complimentary Filter");
Serial.print(b,DEC);

// 330 Grad

if(angle > 0) rundung = (int) (angle + 0.5);                          // Somit werden Werte die fast 0 sein vermieden
...
Serial.print("Rundung");
Serial.println(rundung, DEC);
